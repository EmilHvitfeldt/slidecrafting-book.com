...
This leads us to the last piece of knowledge about CSS fragments.
...
## Sliding code out and results in

The examples we have seen so far only changes the color of things on the slides.
A fragment can animate any property though,
one of those properties are positions.
this section will show how we can use fragments to shuffle some elements around.

Normally when we want to show two things side by side we reach for `columns`,
which comes with the unfortunate downside is that the aspect ratio of the things we want to show being changed so both will fit on the slide.
If we want to show two things side by side in a way that shows things fully we need to get creative.
I will show how we can use fragments to move things around to solve this problem.
For this example we have code on one side,
then the produced output on the righthand side.
...
Both children fill the stage completely,
so the only thing that distinguishes them is their `left` value.
That is the property our two fragments animate:
...
The `opacity: unset; visibility: unset;` pair does the same job it did in the earlier examples:
both panes are on screen the whole time, and only their position responds to the click.
`130%` parks a full-width pane outside the viewport with room to spare.
...
![
  ...
  examples/fragments/fragment-stage.html
){
  .slide-deck
  loading="lazy"
  width="560"
  height="373"
  title="Code sliding out and an embedded deck sliding in"
}

[{{< fa file >}}qmd](examples/fragments/fragment-stage.qmd){.listing-slides .btn-links target="_blank"}
[{{< fa brands sass >}}scss](examples/fragments/stage.scss){.listing-video .btn-links target="_blank"}

For this to work we need them fragments to share an index,
this way they move at the same time.
